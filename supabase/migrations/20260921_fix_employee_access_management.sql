-- Pastikan RPC edit akses tersedia pada database yang sudah terlanjur dibuat.
-- Aman dijalankan ulang; reset password tetap dilakukan oleh Edge Function.
create or replace function public.admin_update_employee_access(
  p_employee_id uuid, p_full_name text, p_position text, p_role text
...
begin
  ...
  if coalesce(trim(p_full_name), '') = '' then raise exception 'Nama lengkap wajib diisi.'; end if;
  if p_role not in ('employee', 'hr', 'finance') then raise exception 'Role yang dipilih tidak valid.'; end if;
  update public.profiles
    set full_name=trim(p_full_name), position=nullif(trim(p_position), ''), role=p_role
    where id=p_employee_id and role <> 'admin'
    returning * into result;
  if not found then raise exception 'Akun tidak ditemukan atau merupakan Super Admin.'; end if;
  return result;
end; $$;
