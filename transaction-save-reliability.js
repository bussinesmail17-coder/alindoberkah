(()=>{
  ...
  async function saveTransaction(){
    ...
    try{
      let response=await window.hmaSupabase.rpc('record_cash_transaction',{p_transaction_date:date,p_type:type,p_category:category,p_description:description,p_amount:nominal,p_employee_id:employeeId});
      if(response.error&&['42883','PGRST202'].includes(response.error.code))response=await window.hmaSupabase.from('cash_transactions').insert({transaction_date:date,type,category,description,amount:nominal,employee_id:employeeId});
      if(response.error)throw response.error;
      form.closest('dialog')?.close();document.getElementById('modalBackdrop')?.classList.remove('show');toast(`${type==='income'?'Pemasukan':'Pengeluaran'} berhasil disimpan. Ringkasan diperbarui otomatis.`);
      document.getElementById('transactionRefresh')?.click();
    }catch(error){toast(`Transaksi belum tersimpan: ${error.message||'periksa izin akun dan koneksi database.'}`)}finally{saving=false;button.disabled=false;button.textContent='Simpan arus kas'}
  }
